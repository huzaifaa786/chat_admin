...
class MessagesController extends Controller
{
    ...
    {
        $query = Chatify::fetchMessagesQuery($request['id'])->latest()->orderBy('created_at', 'asc');
    ...
    public function search(Request $request)
    {
        ...
        foreach ($records->items() as $index => $record) {
            $mergedRecord = $record->toArray() + Chatify::getUserWithAvatar($record)->toArray();
            $mergedRecords[] = $mergedRecord;
        }

        $records->setCollection(collect($mergedRecords));
        return Response::json([
